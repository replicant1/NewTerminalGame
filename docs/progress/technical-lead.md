...
02:31:47Z  VERIFY  seven-capability sweep complete: six of seven wired and called, the seventh (WI-7 skeleton) correctly unreachable because section 7 says WI-14 supersedes its fixtures - and there is a test recording that so nobody fixes it by wiring it back in. Placement was the only real failure
02:31:47Z  CONTRADICT my own M4 scheduled WI-20 and WI-19 in parallel, with WI-20 finishing FIRST. WI-20 deliverable is a count and WI-19 can change code after it - a stale count by construction. Missing edge WI-19 -> WI-20. Fixed: WI-20 now runs last, M4 ends a day later, project 23 days
02:31:47Z  DECIDE  a guard needs a control exactly as a fixture does - a guard whose job is catching something invisible passes when it finds nothing, INCLUDING when the reason is that the guard itself is broken. Pin the positive: a call that is made is reported, a call that is not is not, and the walk reaches everything it claims to cover
02:31:47Z  NOTE    eighth entry for section 1.6 - never assert against a value the environment supplied. A test pinned an unplaced window at (5,38) and failed at x=150, because that is the FIRST toplevel default and a second cascades; pinning it would have made the test depend on how many windows the suite had built first
02:31:47Z  NOTE    second hidden price on human item 4: the placement code swallows every exception deliberately, so a reader wired with the wrong shape is indistinguishable from one that found nothing - it silently centres. Proved by misspelling one stub method and getting five green-looking fallbacks instead of five errors
