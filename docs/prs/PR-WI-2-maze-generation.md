...
**Branch:** `r8/wi-2-maze-generation`, cut from `main` at `8bc9496`. **Lane B**, Dev B. **Realises:** MAZE-1 (the grid), MAZE-2, MAZE-3, MAZE-4, MAZE-5, MAZE-6.
...
- `terminal_game/domain/maze_generator.py`: `generate_maze(rng) -> Maze`. It uses only the source it is handed (anything with `randrange` and `choice`, e.g. `random.Random(seed)`). Carve, then repair, on a lattice. Rooms sit at odd (col, row). A randomised depth-first search opens a spanning tree over the 9 × 14 rooms. Each room left with one open link then gets one more, chosen at random. Squares with both coordinates even, and the outer ring, are never opened. The module docstring explains why each invariant holds by construction. The tests check each one over 1,000 seeds anyway.
...
|---|---|---|---|---|
| WI-2/C1 | Every generated maze is 19 grid squares across and 29 deep, and every square is either wall or corridor. | Executable: `.venv/bin/python -m pytest -v tests/test_maze_generator.py::test_c1_every_maze_is_19_by_29_and_every_square_is_wall_or_corridor` and H | `PASSED`. H: `C1  1000/1000 mazes are 19 x 29; squares that are neither '#' nor '.': 0  PASS`. The test also asserts `is_wall != is_corridor` on all 551 squares of every maze. | n/a: `terminal_game/` does not exist at base `8bc9496` (`git ls-tree -r 8bc9496 terminal_game` is empty). The maze is new code with no counterpart. |
...
terminal_game/domain/maze.py:33-46                  -> WI-2/A3 (immutable value, equality, refusals), C6 (equality is what "same maze" is judged by)
terminal_game/domain/maze.py:48-71                  -> WI-2/A3 (from_rows/to_rows), C1 (every square is '#' or '.')
terminal_game/domain/maze.py:73-97                  -> WI-2/A3 (contains, is_wall, is_corridor, neighbours, open_neighbours, corridor_squares), C1 (is_wall != is_corridor)
terminal_game/domain/maze_generator.py:1-30         -> docstring: why C2-C5 and C7 hold by construction
terminal_game/domain/maze_generator.py:32-47        -> WI-2/A2 (RandomSource protocol; no random import)
terminal_game/domain/maze_generator.py:49-64        -> WI-2/C1, C2, C5 (room lattice inside the ring; pillars never opened)
terminal_game/domain/maze_generator.py:67-84        -> WI-2/C4, C6, C7 (spanning-tree carve, deterministic in rng, bounded)
terminal_game/domain/maze_generator.py:86-93        -> WI-2/C3 (dead-end repair), C4 (only adds links)
tests/test_maze_generator.py (new)                  -> evidence for C1-C7, A1, A2
...
- `from terminal_game.domain.maze import Maze, Square, WIDTH, HEIGHT, DIRECTIONS`. `from terminal_game.domain.maze_generator import generate_maze`.
