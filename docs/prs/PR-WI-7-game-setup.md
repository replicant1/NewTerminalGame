...
| WI-7/C3 | When the centre square is itself corridor, the player starts on it. | Executable: `.venv/bin/python -m pytest -v tests/test_game_setup.py -k c3`, and H | Two tests `PASSED`. One uses a hand-built maze with an open centre. The other covers every generated maze whose centre is corridor, and fails if fewer than 100 are found. H: `C3  mazes with an open centre: 569; player not on it: 0  PASS` |
