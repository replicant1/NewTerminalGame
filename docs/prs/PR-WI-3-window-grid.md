...
**Base:** `main` at `8bc9496`. Soft dependency on WI-1 (#122): `main` is merged in once WI-1 lands, before verification is requested.
...
Every control is `n/a`: **`terminal_game/` does not exist at the base `8bc9496`**, so the shell has no counterpart there to run against. An overlay would fail on `ImportError`, which proves nothing. Where the evidence depends on a *technique* (a posted drag, a pixel mask, a source scan), I give that technique its own positive control in the same run. Those are listed under each claim.
...
```
terminal_game/__init__.py                      -> package marker (mechanical; WI-1 adds the same file, resolved at merge)
terminal_game/shell/__init__.py                -> WI-3/C8 (the characters-only rule written where a developer meets it)
...
- **F4. The key tests depend on nobody using the desktop meanwhile.** In one development run the application stopped being key after 5 s of idling, and every later posted key was dropped. The cause was not established; the likeliest is activity on the shared desktop. The tests now check focus first and fail with that explanation, not with a wrong key.
