...
| WI-11/C11 | Over 1,000 games of 1,000 random moves each, the player never stands on a wall and never leaves the maze. | Executable: `-k c11_`, and H | `PASSED`, asserting at every move that the player is inside 19 × 29, on corridor, and moved by at most one square north, south, east or west. The test fails if fewer than 100,000 moves changed square. H: `C11  1000 games x 1000 random moves: squares changed 509273; off the grid 0; on a wall 0; jumps of more than one square 0  PASS`. The ghost does not step in these games, so they run longer. A game still ends if the player walks into the ghost. |
