...
**Budget for it in the schedule.** A MEDIUM or HIGH work item carries at least one review round and may carry several — the review of the change that introduced this gate took three, and every finding in it was accepted. There is no fixed number to plan against, so budget two rounds and treat a third as ordinary; a HIGH one also has its suite run a second time by the reviewer. If your gantt chart and per-iteration effort totals assume a work item ends when its tests go green, they are wrong by that margin. Say in the plan what you have allowed.

**A review that stops converging comes to you.** The reviewer posts a `BLOCKED` verdict naming the comments still outstanding and the developer's position on each, and it reaches you through the conductor. **Rounds are not what triggers it** — a review that takes several rounds while each round's findings are accepted and fixed is working, and is not yours to hurry. What triggers it is a disagreement that has had its exchange, a round in which nothing moved — nothing new entered the loop, rather than nothing being settled — or the six-round backstop, and the marker on the verdict says which. The first two are a design disagreement wearing a review's clothes — where a responsibility belongs, which interface survives, whether a test proves anything. Answer it, or take it to the user. Those are among the few things in this workflow genuinely yours to decide, because both parties have said their piece and neither can settle it. **The backstop is not that** — it means the two tests failed to name what was wrong, and what it needs from you is a look at the review rather than a ruling on a disagreement.
