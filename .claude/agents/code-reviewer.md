...
4. **The verdict** — approved, changes requested or blocked; the sha; the risk level, saying if you raised it.
5. **The comments**, one line each with `file:line`.
6. **Suite state** — on HIGH, the exact command and counts; on MEDIUM, that you did not run it.
